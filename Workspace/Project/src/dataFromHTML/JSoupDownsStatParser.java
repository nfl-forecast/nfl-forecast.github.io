...
public class JSoupDownsStatParser {
	public static ArrayList<String>[] get(String season) throws IOException {
		...
		usingStats.add("3rd Att");
		usingStats.add("3rd Md");
		Document htmlFile = Jsoup.connect("https://www.nfl.com/stats/team-stats/offense/downs/" + season+ "/reg/all").get();
		...
		for (int i = 0; i < teamRows.size(); i++) {
			...
			for (int j = 1; j < stats.size(); j++) {
				if (usingStats.contains(headerLabels.get(j)))
					passStats[i].add("\"" + headerLabels.get(j).replaceAll(" ", "") + "\": {\"abbreviation\": \"" + headerLabels.get(j) + "\", \"value\": \"" + stats.get(j).ownText() + "\"}");
			}
		}
		return passStats;
	}
