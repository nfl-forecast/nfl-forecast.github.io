...
public class JSoupPassStatParser {
	public static ArrayList<String>[] get(String season) throws IOException {
		...
		usingStats.add("Att");
		usingStats.add("Cmp %");
		usingStats.add("Yds/Att");
		usingStats.add("TD");
		usingStats.add("INT");
		usingStats.add("Sck");
		usingStats.add("SckY");
		Document htmlFile = Jsoup.connect("https://www.nfl.com/stats/team-stats/offense/passing/" + season+ "/reg/all").get();
		...
		for (int i = 0; i < teamRows.size(); i++) {
			...
			for (int j = 1; j < stats.size(); j++) {
				if (usingStats.contains(headerLabels.get(j)))
					passStats[i].add("\"Pass" + headerLabels.get(j).replaceAll(" ", "") + "\": {\"abbreviation\": \"" + headerLabels.get(j) + "\", \"value\": \"" + stats.get(j).ownText() + "\"}");
			}
		}
		return passStats;
	}
